...
function Call() {
  ...
  const handleAddJoinForm = () => {
    ...
    clients.length
      ? setClients([...clients, newChannel])
      : setClients([newChannel]);
  };

  const handleJoinRequest = async (data: JoinData) => {
    if (data.appid && data.channel) {
      setPublishing(true);
      join(
        data.client,
        data.appid,
  ...
  return (
    <div className="call">
      <button
        ref={addFormButton}
        className="aditional-form-button"
        onClick={handleAddJoinForm}
      >
      ...
      {clients.map((cl, index) => {
        return (
          <JoinForm
            returnData={handleJoinRequest}
            returnLeave={handleLeaveRequest}
            client={cl}
            key={index}
          ></JoinForm>
        );
      ...
      <div className="player-container">
        <div className="local-player-wrapper">
          <p className="local-player-text">
            ...
            {localVideoTrack ? `(${publishingClient?.uid})` : ""}
          </p>
          <MediaPlayer
          ...
          {localVideoTrack ? (
            <div className="controll-buttons">
              ...
              >
                <i
                  className={
                    muteVideoState ? "fas fa-video" : "fas fa-video-slash"
                  }
                ></i>
              </div>
              <div
